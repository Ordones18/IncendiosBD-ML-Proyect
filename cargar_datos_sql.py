...
try:
    ...
    print("Ciudades insertadas con éxito.")
except Exception as e:
    print(f"Error al insertar ciudades: {e}")
    raise e

# 3. Cargar y Preprocesar Clima Diario
if os.path.exists('clima_diario_4ciudades.csv'):
    ...
    # Usamos ffill() y bfill() para rellenar los nulos con días adyacentes
    ...
    print(f"Clima insertado con éxito en {time.time() - t0:.2f} segundos.")
else:
    print("Error: clima_diario_4ciudades.csv no encontrado.")

# 4. Cargar y Preprocesar NDVI
if os.path.exists('NDVI-Ecuador-Cities-MOD13Q1-061-results.csv'):
    print("\nProcesando NDVI...")
    ...
    # Agregar columna 'evi' como NULL ya que no está en el dataset
    df_ndvi['evi'] = None
    
    # Filtrar filas donde la ciudad sea válida
...
if os.path.exists('fire_archive_M-C61_761089.csv'):
    ...
    # Cálculo vectorial de distancias a las 4 ciudades
    print("- Calculando ciudad más cercana geográficamente para cada detección...")
    dists = []
...
# 6. Comprobación final y conteo de registros en la base de datos
...
try:
    with engine.connect() as conn:
        ...
        if r_inc >= 10000:
            print("\n¡ÉXITO! Se cumple con el requisito de más de 10,000 registros para la investigación.")
        else:
